        ...
        */
		...
		inline std::string help_pb(const po::options_description &desc, const field_map &fields) {
			...
			BOOST_FOREACH(const boost::shared_ptr<po::option_description> op, desc.options()) {
				...
				std::string tdesc =op->description();
				std::string::size_type pos = tdesc.find("\n");
				if (pos == std::string::npos)
					detail->set_short_description(tdesc);
				else
					detail->set_short_description(tdesc.substr(0, pos));
				detail->set_long_description(tdesc);
			}
